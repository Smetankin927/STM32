...
#elif defined SYSCLK_FREQ_EXT_HSE_PLL
  const uint32_t SystemFrequency         = SYSCLK_FREQ_EXT_HSE_PLL;      /*!< System Clock Frequency (Core Clock) */
  const uint32_t SystemFrequency_SysClk  = SYSCLK_FREQ_EXT_HSE_PLL;      /*!< System clock                        */
  const uint32_t SystemFrequency_AHBClk  = SYSCLK_FREQ_EXT_HSE_PLL;      /*!< AHB System bus speed                */
  const uint32_t SystemFrequency_APB1Clk = (SYSCLK_FREQ_EXT_HSE_PLL/2);  /*!< APB Peripheral bus 1 (low)  speed   */
  const uint32_t SystemFrequency_APB2Clk = SYSCLK_FREQ_EXT_HSE_PLL;      /*!< APB Peripheral bus 2 (high) speed   */
...
#elif defined SYSCLK_FREQ_EXT_HSE_PLL
  static void SetSysClockToEXT_HSE_PLL(void);
#endif
...
#elif defined SYSCLK_FREQ_EXT_HSE_PLL
/**
  * @brief  Selects PLL'd HSE as System clock source and configure HCLK, PCLK2
  *         and PCLK1 prescalers. Also enables the RCC_CR_HSEBYP bit, which
  *         disables the crystal/ceramic resonator.
  * @note   This function should be used only after reset.
  * @param  None
  * @retval None
  */
static void SetSysClockToEXT_HSE_PLL(void)
{
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    uint32_t pll_mul = ((F_CPU / F_CRYSTAL) - 2) << 18;
    ...
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE |
                                        RCC_CFGR_PLLMULL));
    RCC->CFGR |= (uint32_t)(RCC_CFGR_PLLSRC_HSE | pll_mul);
#endif /* STM32F10X_CL */
